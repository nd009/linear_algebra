...
def generatePoints3D(seed, num=100):
    np.random.seed(seed)
    X = np.linspace(-5, 5, num)
    m = np.random.random(2) * 10 - 5
    b = np.random.random() * 15 + 5
    X = (np.vstack([X]*2).T + np.random.randn(num, 2)).tolist()
    Y = (np.dot(X, m) + np.random.randn(num)*5 + b).tolist()
    return X, Y

def vs_scatter(X, Y, m=None, b=None):
    x_vals = (-5, 5)
    plt.xlim(x_vals)
